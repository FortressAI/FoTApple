// FoTLegalApp_DemoMode.swift
// DEMO VERSION - Always shows Siri-guided onboarding for video recording
// To use: Rename this to FoTLegalApp.swift (backup the original first)
...
struct FoTLegalApp: App {
    ...
    // DEMO MODE: Never persist onboarding state
    @State private var currentlyInOnboarding: Bool = true
    
    init() {
        AppConfig.shared.features.useLocalLLM = false
        AppConfig.shared.features.vqbitSuggestions = true
        FoTLogger.app.info("FoT Legal US starting - 🎬 DEMO MODE with Siri Voice")
    }
        ...
        WindowGroup {
            if currentlyInOnboarding {
                LegalOnboardingFlow {
                    ...
                    // After 5 seconds in main app, restart onboarding
                    // (for continuous demo recording)
                    DispatchQueue.main.asyncAfter(deadline: .now() + 5.0) {
                        withAnimation {
                            currentlyInOnboarding = true
                        }
                    }
                }
            } else {
